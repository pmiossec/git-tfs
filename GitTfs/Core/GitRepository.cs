...
namespace Sep.Git.Tfs.Core
{
    public class GitRepository : GitHelpers, IGitRepository
    {
        ...
        public static bool TryParseChangesetId(string commitMessage, out int changesetId, out string tfsPath)
        {
            var match = tfsIdRegex.Match(commitMessage);
            if (match.Success)
            {
                changesetId = int.Parse(match.Groups[3].Value);
        ...
        private Commit FindCommitByChangesetId(int changesetId, string remoteRef = null, string tfsPath = null)
        {
